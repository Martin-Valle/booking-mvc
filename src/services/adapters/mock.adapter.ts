...
const restaurants: Restaurant[] = [
  {
    id: "r1",
    name: "Casa Tapas Madrid",
    city: "Madrid",
    country: "España",
    pricePerPerson: 18,
    rating: 4.5,
    photo: "/assets/rest1.jpg",
    cuisineTags: ["tapas", "española"],
  ...
  },
  {
    ...
    pricePerPerson: 25,
    rating: 4.2,
    photo: "/assets/rest2.jpg",
    cuisineTags: ["mariscos", "mediterránea"],
    distanceCenterKm: 1.4,
  },
  {
    id: "r3",
    name: "El Rincón Quiteño",
    city: "Quito",
    country: "Ecuador",
    pricePerPerson: 10,
    rating: 4.3,
    photo: "/assets/rest3.jpg",
    cuisineTags: ["ecuatoriana"],
...
// ✅ Deja un único export para restaurante por ID
...
export async function mockSearch(q: string): Promise<SearchResult[]> {
  ...
  const H = hotels
    .filter((h) =>
      `${h.name} ${h.city} ${h.country ?? ""}`.toLowerCase().includes(term)
    )
    .map((h) => ({ kind: "hotel", item: h }) as SearchResult);

  const C = cars
    .filter((c) =>
      `${c.brand} ${c.model} ${c.city ?? ""} ${c.country ?? ""}`
        .toLowerCase()
        .includes(term)
    )
  ...
  const F = flights
    .filter((f) =>
      `${iataText(f.from)} ${iataText(f.to)} ${f.airline}`
        .toLowerCase()
        .includes(term)
    )
    .map((f) => ({ kind: "flight", item: f }) as SearchResult);

  // ✅ Solo una vez: R. Y usa cuisineTags (no "cuisine")
  const R = restaurants
    .filter((r) =>
      `${r.name} ${r.city} ${r.country ?? ""} ${(r.cuisineTags ?? []).join(" ")}`
        .toLowerCase()
        .includes(term)
    )
